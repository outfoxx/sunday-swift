...
class NetworkRequestAdapterTests: XCTestCase {
  ...
  func testHostMatchingAdapter() throws {
    ...
    let setMatched$ =
      setAdapater.adapt(requestFactory: Self.requestFactory, urlRequest: URLRequest(url: URL(string: "http://api.example.com")!))
      .record()
    ...
    let setUnatched$ =
      adapter.adapt(requestFactory: Self.requestFactory, urlRequest: URLRequest(url: URL(string: "http://other.example.com")!))
      .record()
  ...
  func testRefreshingTokenAuth() throws {
    ...
    let refresher = { (rf: NetworkRequestFactory) -> RefreshingHeaderTokenAuthorizingAdapter.RefreshResult in
